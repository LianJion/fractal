...
function drawWater() {
  ...
  dropWaterOffset = dropWaterOffset < 500 ? dropWaterOffset + FAST_TREE_VELOCITY/fps : 500;
  if (dropWaterOffset < 500 ) {
    // koch.update(kochmvcopy);
    // kochsnowflake.update(kochmvcopy);

  } 
  canvas.ctx.save();
    canvas.ctx.translate(dropWaterOffset, 0);
...
function drawIce() {
  ...
  //改了与冰棒之间的间距
    canvas.ctx.drawImage(snow, snowpos.x, snowpos.y, snow.width/4, snow.height/4);
}
